...
const PLANETS: Record<PlanetName, { el: Elements; rate: Elements }> = {
  mercury: {
...
export function planet(name: PlanetName, t: number, orbitEpoch = t): PlanetState {
  const T = centuries(t);
  const { el, rate } = PLANETS[name];
  const [a, e, I, L, wbar, node] = el.map((v, i) => v + rate[i] * (i === 3 ? T : centuries(orbitEpoch))) as Elements;
  const w = wbar - node;
  const M = norm(L - wbar);
  // Kepler's equation
  let E = M + R2D * e * sin(M);
